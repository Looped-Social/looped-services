...
public class LikesController {
    ...
    public ResponseEntity<?> like(@AuthenticationPrincipal Jwt jwt, @PathVariable("postId") long postId,
        ...
        return switch (res.status()) {
            ...
            case NOT_VERIFIED -> ResponseEntity.status(HttpStatus.FORBIDDEN).body(
                    lockDeniedBody("community_not_verified", "You must be verified to react in this community", res.lock())
            );
            ...
            case SPECIALIZATION_NOT_JOINED -> ResponseEntity.status(HttpStatus.FORBIDDEN).body(
                    lockDeniedBody("specialization_not_joined", "You must join this specialization to react", res.lock())
            );
    ...
    @DeleteMapping
    public ResponseEntity<?> unlike(@AuthenticationPrincipal Jwt jwt, @PathVariable("postId") long postId,
                                    @RequestBody(required = false) LikeRequest body) {
        boolean asAnon = body != null && Boolean.TRUE.equals(body.asAnon());
        if (asAnon && (body == null || !body.hasAnonProof())) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN).body(Map.of(
        ...
        var res = service.unlike(jwt == null ? null : jwt.getSubject(), postId, body == null ? null : body.toAnonProof());
        return switch (res.status()) {
            case USER_NOT_PROVISIONED -> ResponseEntity.status(HttpStatus.CONFLICT).body(Map.of(
                    "error", "user_not_provisioned",
                    "message", "Complete onboarding before reacting"
