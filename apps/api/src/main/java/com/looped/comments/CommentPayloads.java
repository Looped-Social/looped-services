...
public final class CommentPayloads {
    ...
    public static Map<String, Object> from(CommentsRepository.CommentViewRow row, String defaultProfileImageUrl) {
        boolean isDeleted = isDeleted(row);
        Map<String, Object> out = new HashMap<>();
        out.put("id", row.comment.id);
        out.put("post_id", row.comment.postId);
        out.put("content", isDeleted ? "" : row.comment.content);
        out.put("media_asset_id", isDeleted ? null : row.comment.mediaAssetId);
        ...
        out.put("author_principal_id", isDeleted ? null : row.author.principalId);
        ...
        author.put("id", isDeleted ? null : authorId(row));
        author.put("principal_id", isDeleted ? null : row.author.principalId);
        author.put("is_anonymous", row.author.isAnonymous);
        author.put("display_name", isDeleted ? null : row.author.displayName);
        author.put("username", isDeleted ? null : row.author.handle);
        author.put("handle", isDeleted ? null : row.author.handle);
        author.put("company_id", isDeleted ? null : row.author.companyId);
        author.put("profile_image_url", isDeleted ? null : com.looped.users.ProfileImageUrls.resolve(row.author.profileImageUrl, defaultProfileImageUrl));
        out.put("author", author);

        return out;
    }

    public static Map<String, Object> publicFrom(CommentsRepository.CommentViewRow row, String defaultProfileImageUrl) {
        boolean isDeleted = isDeleted(row);
        Map<String, Object> out = new HashMap<>();
        out.put("id", row.comment.id);
        out.put("post_id", row.comment.postId);
    ...
    private static Long authorId(CommentsRepository.CommentViewRow row) {
        return row.author.userId != null ? row.author.userId : row.author.anonProfileId;
    }

    private static boolean isDeleted(CommentsRepository.CommentViewRow row) {
        return row.comment.deletedAt != null || row.comment.removedAt != null;
    }
}
